...
class WithdrawalRecord extends StatefulWidget {
  @override
  _WithdrawalRecordState createState() => _WithdrawalRecordState();
}

class _WithdrawalRecordState extends State<WithdrawalRecord> {
  int _page = 0;
  ...
  void getList() async {
    ...
    UserServer().getTxRecord(map, (success) async {
      setState(() {
        isLoading = false;
        if (_page == 1) {
          //赋值
              ...
              child: new Row(children: <Widget>[
                ...
                Expanded(
                  ...
                  child: Container(
                    alignment: Alignment.center,
                    child: Text(
                      item['status'] == '0'?'审核中':item['status'] == '1'?'已到账':item['status'] == '2'?'提现失败':'',
                      style: TextStyle(
                        color: Color(0xffE71419),
                        fontSize: ScreenUtil().setSp(28),
                      ),
